...
##############################################
# Recipient Functions (Data Preparation & Training)
##############################################

def get_recipient_label(folder_name):
    """
    Extract the recipient label from folder names like:
    'BMR2_vs_BMR3_28' or 'BMR2_vs_BMR3_35'.

    We do this by:
      1. Splitting on the substring '_vs_'.
      2. Taking the second part (e.g., 'BMR3_28').
      3. Splitting on '_' again to isolate 'BMR3' from '28'.
      4. Return the first element ('BMR3') as the recipient label.
    """
    ...
    # The second part might look like 'BMR3_28'
    second_part = parts[1]
    subparts = second_part.split('_')
    # The recipient code is the first subpart, e.g. 'BMR3'
    return subparts[0]  # 'BMR3'

def process_all_files_recipient(root_directory, cutoff):
    """
    Walk through all subdirectories and process each .wav file.
    The recipient label is extracted using get_recipient_label().
    """
    processed_data = {}
...
def prepare_dataset_recipient(filtered_signals_dict, exclude_label, use_augmentation=False):
    """
    Prepare the Recipient dataset.
    Exclude samples from subfolders whose recipient label equals `exclude_label` in training,
    ...
    for subfolder in train_subfolders:
        recipient_label = get_recipient_label(subfolder)
        if recipient_label == exclude_label:
            continue
        for filename in filtered_signals_dict[subfolder]:
            ...
            X.append(features)
            y.append(recipient_label)
            train_signals.append((sample_rate, filtered_signal, recipient_label))
...
def train_recipient_model(exclude_label, use_augmentation=False):
    """Train the Recipient RandomForest model with hyperparameter tuning."""
    root_directory = r"C:\signals"  # Adjust the path as needed
    cutoff = 500.0

    print("=== Recipient: Gathering data ===")
    filtered_signals_dict = process_all_files_recipient(root_directory, cutoff)
    print(f"[Recipient] Total subfolders processed: {len(filtered_signals_dict)}")

    # Display number of examples per label before splitting
    recipient_counts = {}
    for subfolder, files_dict in filtered_signals_dict.items():
        recipient_label = get_recipient_label(subfolder)
        recipient_counts[recipient_label] = recipient_counts.get(recipient_label, 0) + len(files_dict)
    print("\n[Recipient] Number of signals per recipient before splitting:")
    for label, count in recipient_counts.items():
        print(f"{label}: {count}")

    print(f"\n=== Recipient: Preparing dataset (excluding {exclude_label} from training) ===")
    X_train, X_test, y_train_enc, y_test_enc, label_encoder = prepare_dataset_recipient(
    ...
    print("\n=== Recipient: Training RandomForest model with hyperparameter tuning ===")
    ...
    print("\n[Recipient] Best Hyperparameters:")
    print(random_search.best_params_)

    y_pred = best_rf_model.predict(X_test)
    rf_accuracy = accuracy_score(y_test_enc, y_pred)
    print(f"\n[Recipient] RandomForest Accuracy on Test Set: {rf_accuracy:.2f}")
    print("[Recipient] Classification Report (RandomForest only):")
    ...
    plt.title('Recipient: Confusion Matrix on Test Set')
...
##############################################
# Group-Level Novelty Detection for Recipient
##############################################

def group_level_novelty_test(rf_model, ocsvm_model, recipient_label_encoder, root_directory, cutoff, threshold=0.65):
    """
    For each unique recipient label:
    ...
    """
    recipient_data = process_all_files_recipient(root_directory, cutoff)
    grouped_features = {}

    # Group signals by recipient label
    for folder in recipient_data:
        label = get_recipient_label(folder)
        for filename in recipient_data[folder]:
            sample_rate, original_signal, filtered_signal = recipient_data[folder][filename]
        ...
        if unknown_percentage > threshold:
            ...
            for idx, pred in enumerate(svm_preds):
                if pred == 1:
                    ...
                    decoded = recipient_label_encoder.inverse_transform(single_pred)[0]
...
##############################################
# Combined System Test for Recipient
##############################################

def combined_system_test_recipient(exclude_label, use_augmentation=False):
    """
    1. Train the Recipient model (RF) and the One-Class SVM.
    ...
    print("========== Combined System Test for Recipient ==========")
    print("Step 1: Training Recipient Model (RandomForest)")
    rf_model, X_train, recipient_label_encoder, X_test, y_test_enc = train_recipient_model(
        exclude_label, use_augmentation=use_augmentation
    )

    print("Step 2: Training One-Class SVM for Recipient (novelty detection)")
    ocsvm_recipient = train_one_class_svm(X_train, nu=0.05, gamma='scale')

    print("\nStep 3: Group-Level Novelty Test")
    group_results = group_level_novelty_test(
        rf_model, ocsvm_recipient, recipient_label_encoder,
    ...
    print("\nStep 4: Full Test on All Recipient Signals (Combined Prediction)")
    filtered_signals_dict = process_all_files_recipient(root_directory, cutoff)
    _, X_test_full, _, y_test_full_enc, label_encoder_full = prepare_dataset_recipient(
    ...
    # Combined predict
    preds_full = combined_predict(rf_model, ocsvm_recipient, X_test_full, recipient_label_encoder, confidence_threshold=0.6)

    print(f"\n[Recipient Model] Full Test on All Signals (excluding '{exclude_label}' and 'Unknown'):")
    ...
    if np.sum(mask) > 0:
        ...
        plt.title('Recipient: Confusion Matrix on Full Test Set (filtered)')
        plt.xlabel('Predicted Label')
        plt.ylabel('True Label')
        plt.show()
    else:
    ...
    print("\n========== Combined System Test for Recipient Completed ==========")
...
def main():
    ...
    filtered_signals_dict = process_all_files_recipient(root_directory, cutoff)
    available_labels = sorted(set([get_recipient_label(folder) for folder in filtered_signals_dict.keys()]))

    print("Available recipient groups for exclusion:")
    for label in available_labels:
        print(f"- {label}")

    exclude_label = input("Please enter the recipient label you wish to exclude from training: ").strip()
    ...
    combined_system_test_recipient(exclude_label, use_augmentation=use_augmentation)
